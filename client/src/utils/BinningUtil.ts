...
export interface ParseGraphDataInterface {
  nodeInMap: NodeMapInterface,
  nodeOutMap: NodeMapInterface,
  nodeNumIn: number,
  nodeNumOut: number,
}
...
export interface DegreeBinBoundariesInterface {
  binIntervalIn: number,
  binIntervalOut: number,
...
export interface HistogramDegreeInterface {
  binArrayIn: Array<number>,
  binArrayOut: Array<number>,
...
const parseGraphData = (
  ...
  config: BinningDegreeConfigInterface,
): ParseGraphDataInterface => {
  const nodeInMap: NodeMapInterface = {};
  const nodeOutMap: NodeMapInterface = {};
...
const binBoundaries = (
  ...
  config: {binCount: number},
): BinBoundariesInterface => {
  const { nodeMap } = args;
  const { binCount } = config;

  const output: BinBoundariesInterface = {
...
const binData = (
  ...
  const binArray: Array<number> = Array(binCount).fill(0);
  const output: BinDataInterface = {
    binArray,
    binMaxVal: 0,
  };
...
export const histogramDegree = (
  graph: GraphInterface,
  configRaw: BinningDegreeConfigInterface,
): HistogramDegreeInterface => {
  const { edges } = graph;
